...
char nextCharWhenWordIsNotInDictionary(const set<char>& selectedChars)
{
    ...
    for(auto j = selectedChars.begin(), i = j++; j != selectedChars.end(); ++i, ++j){
        if((*i) + 1 != (*j)){
            answer = char((*i) + 1);
            break;
        }
...
map<char, int> countOccurrences(const vector<string>& candidateWords)
{
    ...
    for(auto s : candidateWords){
        for(int i = 0; i < (int)s.size(); i++)answer[s[i]]++;
    }
    return answer;
}
...
char findMostFrequentChar(const map<char, int>& occurrences, const set<char>& selectedChars)
{
    ...
    //Write your code here
    int mostFrequent = 0;
    for(auto character : occurrences){
        if(selectedChars.find(character.first) == selectedChars.end()){
            if(mostFrequent < character.second){
                mostFrequent = character.second;
...
bool isCorrectChar(char ch, const string& mask)
{
    ...
    //Write your code here
    answer = 0;
    for(int i = 0; i < (int)mask.size(); i++)if(ch == mask[i])answer = 1;
    return answer;
}
...
bool isWholeWord(const string& mask)
{
    ...
    //Write your code here
    answer = 1;
    for(auto ch : mask)if(ch == '_')answer = 0;
    return answer;
}
...
bool wordConformToMask(const string& word, const string& mask, char ch)
{
    bool answer;
    //Write your code here
    answer = (mask.size() == word.size());
    for(int i = 0; i < (int)mask.size(); i++){
        if(mask[i] == '_')continue;
        answer &= (mask[i] == word[i]);
    }
    for(int i = 0; i < (int)word.size(); i++){
        if(ch == word[i])return answer;
    }
    return 0;
    return answer;
}
